...
export function MainNav() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 glass" role="banner">
      <nav aria-label="Main navigation" className="flex items-center justify-between w-full py-4 px-4 md:px-8 max-w-7xl mx-auto">
        <button
        ...
        >
          ...
          <div>
            ...
            <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-primary-500 to-secondary-500 group-hover:w-full transition-all duration-300" aria-hidden="true"></span>
          </div>
        </button>
          ...
          {routes.map((route) => (
            <li key={route.href}>
              <button
                ...
                aria-label={`Navigate to ${route.label} section`}
              ...
              >
                ...
                <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-primary-500 group-hover:w-full transition-all duration-300" aria-hidden="true"></span>
              </button>
            </li>
          ))}
        </ul>
          ...
          <Sheet>
            ...
            <SheetContent side="right" className="w-72 max-w-[90vw] bg-gradient-subtle backdrop-blur-md border-l border-border/50">
              ...
              <nav aria-label="Mobile navigation">
                <ul className="flex flex-col gap-2 py-6 px-6" role="list">
                  {routes.map((route) => (
                    <li key={route.href}>
                      <button
                        onClick={() => {
